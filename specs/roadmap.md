...
Settle every open question in `mission.md` and record the result in
`specs/carioca-rules.md`: contract sequence, run length, deck composition, joker
handling, laying down, scoring, win condition. Include a worked example of one
full hand with scores.
...
### Phase 2 — Cards and decks
Card representation, suits, ranks, jokers. Deck construction for a given player
count. Seeded shuffle. Dealing.
**Done when:** the same seed always produces the same deal, proven by a test.

### Phase 3 — Melds
Validate a trio and a run, including jokers standing in for a missing card, and
reject the invalid cases from the rules document.
**Done when:** a table-driven test covers every valid and invalid meld example in
`carioca-rules.md`.

### Phase 4 — One hand, start to finish
Turn order, draw from stock or discard, lay down a contract, discard, and detect
going out. Illegal moves are rejected by the engine, not by the caller.
**Done when:** a test plays a scripted hand to completion and the final state is
correct.

### Phase 5 — Full game and scoring
The contract sequence across all hands, per-hand scoring, cumulative totals, and
the win condition.
**Done when:** a test plays a complete game from a seed and produces a final
...
### Phase 6 — A bot that can finish a game
...
mutate one.
**Done when:** a finished game state from Phase 5 renders correctly on a phone
screen.

### Phase 8 — Play it
Wire interaction: draw, select cards, lay down, discard. Local game against the
Phase 6 bots, state in Zustand.
...
### Phase 14 — Server-authoritative play
